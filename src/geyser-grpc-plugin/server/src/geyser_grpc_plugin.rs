...
impl GeyserPlugin for GeyserGrpcPlugin {
    ...
    fn update_account(
        ...
        let my_data = &account_update.account_update.as_ref().unwrap().data;

        let x = bs58::encode(&pubkey).into_string();
        let y = bs58::encode(&owner).into_string();
        info!("gaurav panwar1 account {:?}", x);
        info!("gaurav panwar1 owner {:?}", y);
        info!("gaurav panwar1 data {:?}", my_data.len());
    ...
    fn notify_transaction(
        ...
        info!("gaurav panwar3 notify {:?}", transaction_update);
